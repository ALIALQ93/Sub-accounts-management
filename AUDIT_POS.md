...
> **حالة المعالجة (مستودع الكود، 2026-07-25):** #1 صلاحية `pos.sell` ✓ (مطبّقة بتعليق صريح "AUDIT_POS.md" داخل `setup_all.sql:15746-15779` — تقبل `pos.sell` كبديل لـ`invoices.post/edit` مقيّداً بـ`pos_point_id is not null`، ونفس النمط بسياسات RLS لإدراج الفاتورة وأسطرها) · #2 تضييق RLS نقاط البيع ✓ (`setup_all.sql:16066-16167`، تعليق صريح "AUDIT_POS.md #2" — سياسات `pos.settings`/`is_admin()` فقط للكتابة، قراءة أوسع بـ`pos.view/pos.sell/pos.settings`) · #3 إنفاذ أعلام النقطة (`allow_price_override`, `allow_line_discount`, `require_customer`, قوائم المواد/الفئات المسموحة) ✓ (`setup_all.sql:15894-15961`، تعليق صريح "AUDIT_POS.md #3" داخل `assert_invoice_may_post` — يرفض الترحيل عند مخالفة أي منها) · **#4 ✓ مُصلَح أيضاً** (تصحيح: كتبت هنا سابقاً اليوم أنه لا يزال مفتوحاً استناداً لـ`materials-warehouses-audit.md §3` دون تحقّق مباشر — عند القراءة المباشرة لآخر تعريف فعلي لـ`inventory_movements_enforce_stock()` بـ`setup_all.sql:18147-18207` تبيّن وجود `pg_advisory_xact_lock(hashtextextended(...))` فعلياً قبل قراءة الرصيد). **كل بنود هذا الملف (1-4) مُغلَقة بالكامل الآن.**
