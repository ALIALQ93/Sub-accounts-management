...
> **حالة المعالجة (مستودع الكود، 2026-07-25):** #4 قفل صف سقف التخصيص عند الترحيل ✓ · #5 تأكيد قبل عكس السند ✓ · #10 `debit_base_base` ✓ (مؤكَّد مسبقاً، لا يزال مُصلَحاً) · **لا تزال مفتوحة:** #1 12 ملف patch يتيمة غير مربوطة بـ`build_setup_all.ps1` · #2 ترويسة `01_schema.sql` مضلِّلة ("الوضع الحالي") · #3 حذف سند مرحّل بدون فحص فترة محاسبية أو صلاحية أدمن · #6 إلغاء السند الأصلي أثناء العكس يتجاوز فحص الفترة المحاسبية (لم يُعالَج رغم إصلاح #4/#5) · #7 RLS مفتوحة تماماً بلا `has_permission()` على الحسابات/السندات/القيود · #8 استخدام `debit`/`credit` الخام بدل الأعمدة القاعدية (`_base`) بفحص سقف التخصيص و`open_items_view` · #9 لا عمود `account_type`/`is_system` (فجوة تصميم).
