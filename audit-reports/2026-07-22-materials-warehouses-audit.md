...
> **حالة المعالجة (مستودع الكود، 2026-07-23 ثم 2026-07-25):** #1–4 و#6–7 ✓ · #5 تنبيه مناقلات عالقة ✓ · #8 توثيق/مواءمة صيغة الوحدات (حدّثت 2026-07-25: `round` إلى 6 + مصدر JS مشترك) ✓ · #9 رفض سطر فاتورة بلا وحدة أساس (#45) ✓ · #12 قائمة تحقق الحركات ✓ · #10 RLS بالفرع و#11 رصيد مُخزَّن معلّقان (معماريان). تأكيد إضافي 2026-07-25: تريغر منع تغيير فرع المستودع موجود فعلياً في `patch_audit_remaining` (#6).
...
## 3. قيد التزامن (Race Condition) — ✓ تم إصلاحه لاحقاً (تصحيح 2026-07-25، راجع سطر 3)

> **تحديث:** هذا القسم كان يصف الوضع وقت كتابة التقرير (2026-07-22). حسب حالة المعالجة بأعلى الملف (سطر 3، #1)، أُصلحت لاحقاً عبر `patch_materials_warehouses_audit_fix.sql` — النسخة الفعّالة الحالية من `inventory_movements_enforce_stock()` (`database/setup_all.sql:18147-18207`، آخر تعريف من 6) تستدعي `perform pg_advisory_xact_lock(hashtextextended(new.material_id::text || '|' || new.warehouse_id::text, 42))` قبل قراءة الرصيد. تحقّقتُ من هذا مباشرة بتاريخ 2026-07-25 بعد تعارض بين هذا الملف وتقارير أخرى استندت لنص هذا القسم دون ملاحظة الإصلاح المذكور بالهيدر. النص الأصلي أدناه محفوظ للسياق التاريخي فقط:

قيد التزامن (Race Condition) — الوصف الأصلي وقت الكتابة، لم يعد ينطبق:
...
6. ~~**تحويل فحص تغيير فرع المستودع إلى قيد/تريغر بقاعدة البيانات**~~ — ✓ مُغلق (`warehouses_prevent_branch_change_with_movements` في `patch_audit_remaining.sql`).
...
8. ~~**توحيد منطق تحويل الوحدات**~~ — ✓ مُغلق 2026-07-25: `computeFactorToBase` يطابق `round(..., 6)` في SQL؛ تعليقات متبادلة؛ المرجع النهائي يبقى التريغر.
