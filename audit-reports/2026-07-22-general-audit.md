...
> **حالة المعالجة (مستودع الكود، 2026-07-25):** #1 قفل تخصيص السندات (`voucher_allocations`) ✓ · #2 قفل تزامن فحص المخزون ✓ (`pg_advisory_xact_lock` أُضيف فعلياً — بعكس ما ذُكر بتقارير أخرى، راجع الملاحظة أدناه) · #3 إنفاذ `max_discount_percent`/`release_on_cancel` ✓ · #4 تأكيد `confirm()` قبل عكس السند ✓ · #5 إنشاء مادة عبر RPC ذرّي ✓ · #6 فهرس القيد الافتتاحي (`branch_id is null`) ✓ · **#7 لا تزال مفتوحة (وأسوأ مما وُصف)** — تشتت `post_invoice()`/`vouchers_before_update_handle_posting()` بين ملفات patch متعددة (الآن 5 نسخ لكل منهما، لا 3). لا اختبارات آلية بالمشروع إطلاقاً (4.2) وملفات ضخمة تزداد حجماً (`invoice-form.tsx` 2083 سطر) لا تزال قائمة.
>
> **تحديث بعد تحقّق يدوي حاسم (نفس الجلسة):** كان هناك تعارض بين هذا الملف و`AUDIT_INVENTORY.md`/`materials-warehouses-audit.md §3` حول قفل تزامن فحص المخزون. تحققتُ مباشرة بقراءة **آخر تعريف فعلي** لدالة `inventory_movements_enforce_stock()` بـ`database/setup_all.sql:18147-18207` (آخر نسخة من أصل 6 تعريفات بالملف، أي الفعّالة فعلياً) — **تحتوي فعلياً** `perform pg_advisory_xact_lock(hashtextextended(new.material_id::text || '|' || new.warehouse_id::text, 42))` قبل قراءة الرصيد (سطر 18180-18182). **القفل موجود فعلاً — نتيجة هذا الملف هي الصحيحة، وقد صحّحتُ `AUDIT_INVENTORY.md` و`AUDIT_POS.md` و`materials-warehouses-audit.md` تبعاً لذلك.**
