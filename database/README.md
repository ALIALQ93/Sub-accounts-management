...
| `setup_demo_restaurant.sql` | **النسخة التجريبية** — نفس `setup_all.sql` بالكامل + بيانات عرض مطعم: قيد افتتاحي مالي `OPEN-*` + فاتورة بضاعة أول المدة `DEMO-OPS-001` ثم سلسلة تشغيل `DEMO-*` (انظر `TRIAL_SETUP.md`). |
...
للقواعد **الموجودة مسبقاً** دون إعادة تثبيت كامل: يمكن تشغيل `database/upgrade_material_stage_expiry.sql` لإضافة أوضاع `semi*` وعمود سياسة صلاحية التصنيع، ثم التأكد من تطابق دوال التصنيع/التفكيك مع `setup_all.sql`.
