...
export interface SalesLinesReportParams {
  fromDate?: string;
  toDate?: string;
  customerId?: string;
  materialId?: string;
...
export const SALES_COMMERCIAL_KIND_LABELS: Record<string, string> = {
  sale: "مبيعات",
  return_sale: "مرتجع مبيعات",
};

function mapRow(row: Record<string, unknown>): SalesLineReportRow {
  return {
...
export const salesLinesReportApi = {
  async listRows(params: SalesLinesReportParams = {}): Promise<SalesLineReportRow[]> {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.rpc("get_sales_lines_report", {
    ...
    if (isMissingRpc(error)) {
      throw new Error(
        "دالة get_sales_lines_report غير متوفرة — شغّل patch_inventory_phase7.sql.",
      );
    }
  ...
  summarize(rows: SalesLineReportRow[]) {
    return rows.reduce(
      (acc, row) => {
        const sign = row.commercial_kind === "return_sale" ? -1 : 1;
        return {
