...
| بعد تعديل أي `patch_*.sql` أثناء البناء | `powershell -File database/build_setup_all.ps1` ثم أعد تشغيل `setup_all.sql` |
| عميل حي ببيانات لاحقاً | شغّل **فقط** ملفات الـpatch الجديدة — **لا** `setup_all` |
