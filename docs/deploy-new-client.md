...
## 2) تهيئة قاعدة البيانات (قسم إلزامي)

مصدر الحقيقة للمخطط في **مرحلة البناء** والعميل الجديد:
...
|--------|----------------|
| بيئة تطوير / بناء / مشروع فارغ | شغّل `setup_all.sql` كاملاً (يحذف ويعيد البناء) |
| عميل حي ببيانات لاحقاً | **لا تشغّل `setup_all.sql`** — لا يوجد نظام ترقيعات بعد 2026-07-27؛ اكتب SQL ترقية يدوي للفروق فقط (راجع §7) |

> **2026-07-27:** حُذف نظام `patch_*.sql`/`build_setup_all.ps1` — `setup_all.sql` أصبح ملفاً واحداً شاملاً يُعدَّل مباشرة، بلا سكربت توليد.

في **SQL Editor** الصق محتوى `setup_all.sql` بالكامل.
...
select public.get_schema_setup_status() ->> 'ok';
-- متوقع: true
```
...
3. أكمل الخطوات: **تهيئة قاعدة البيانات** → الشركة → المدير → الفرع/المستودع → قبول دليل الحسابات → المخزون → (تخطي القيد الافتتاحي) → إتمام.
4. خطوة «تهيئة قاعدة البيانات» تستدعي `get_schema_setup_status()`؛ إن فشلت أعد تشغيل `setup_all.sql` ثم «إعادة التحقق».
5. بعد الإتمام تُفتح الشاشة الرئيسية ولا يُعاد فتح الويزارد.
...
عند إصدار ميزة جديدة تتطلب SQL (لا يوجد نظام ترقيعات منفصل بعد 2026-07-27 — التعديل يكون مباشرة بـ`setup_all.sql`):

1. عدّل `database/setup_all.sql` مباشرة (أضف/غيّر الجداول/الدوال/المحفزات اللازمة)، وطبّق نفس التعديل يدوياً على `database/setup_demo_restaurant.sql` إن كان يخص الجزء المشترك (وليس بيانات العرض فقط).
2. على بيئة تطوير/بناء فارغة: شغّل `setup_all.sql` كاملاً للتأكد أنه يُنفَّذ بلا أخطاء من الصفر.
3. على **كل** مشروع عميل حي: **لا تشغّل `setup_all.sql`** — استخرج فقط SQL الفرق الجديد (الجدول/الدالة/العمود المُضاف) من `setup_all.sql` وشغّله يدوياً بمفرده على قاعدة العميل.
4. انشر نسخة التطبيق الجديدة على بيئة كل عميل (نفس الـcommit أو الوسم).
5. دخّل كمدير وتحقق من مسار دخان قصير (دخول، قائمة، عملية واحدة أساسية للميزة).
6. سجّل تاريخ الترقية وملخص SQL المُطبَّق يدوياً بمذكرة تشغيل العميل.
...
- [ ] `setup_all.sql` نُفِّذ والتحقق (§2) ناجح — بما فيه `get_schema_setup_status() -> ok`
- [ ] Email Auth + مستخدم مدير أول
- [ ] env التطبيق مضبوط ومنشور (Root = `web`)
- [ ] ويزارد `/setup` مكتمل (يشمل خطوة تهيئة قاعدة البيانات)
